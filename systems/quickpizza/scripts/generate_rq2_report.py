...
REPORT_PATH = REPORTS_DIR / "rq2_quickpizza_report.html"
SYSTEM_NAME = "Grafana QuickPizza"
DESCRIPTION = "Grafana QuickPizza is a pizza recommendation microservice demo with a Grafana-native local observability stack. For RQ2 it tests whether the RL policies still react coherently when the same adaptive tracing workflow is moved onto a fourth application."
CAVEAT = "QuickPizza should still be interpreted carefully because scrape health across the full telemetry stack is not yet completely clean, even though the corrected runtime-change matrix is now available."
...
SCENARIO_TEXT = {
    "healthy": "Baseline application behavior with standard POST traffic to /api/pizza and no injected delay or error headers.",
    "latency_spike": "Request headers enforce deterministic recommendation delay to increase end-to-end latency without intentionally increasing the error rate.",
    "error_burst": "Request headers inject deterministic ingredient-fetch failures that propagate back to the public API as observable error responses.",
    "throughput_drop": "Worker count and pause interval are reduced to intentionally lower achieved throughput.",
}
...
def bar_chart(rows, scenario, key, label, lower_is_better):
    ...
    ceiling = max((row[key] for row in scoped), default=1.0) or 1.0
    blocks = []
    for row in scoped:
...
def scenario_table(rows, scenario):
    ...
    parts = [
        "<table><thead><tr><th>Policy</th><th>Latency (ms)</th><th>QPS</th><th>Error Rate</th><th>Avg Reward</th><th>Avg Rate</th><th>Decisions</th><th>Zero Windows</th></tr></thead><tbody>"
    ]
    for row in scoped:
        parts.append(
...
def build_html(rows):
    ...
    scenario_definition_cards = "".join(
        f"<article class='card summary'><div class='eyebrow'>{title}</div><p>{SCENARIO_TEXT[key]}</p></article>"
