...
REPORT_PATH = REPORTS_DIR / "rq2_spring_petclinic_microservices_report.html"
SYSTEM_NAME = "Spring Petclinic Microservices"
DESCRIPTION = "Spring Petclinic Microservices is the business-domain application in the study. For RQ2 it shows whether RL policies react consistently when the runtime signal is noisier and decision windows are longer."
CAVEAT = "This system is the least stable of the three. Low decision counts or sparse windows should be treated as a reporting limitation, not ignored."
...
SCENARIO_TEXT = {"healthy": "Baseline operating condition with the standard gateway traffic profile.", "latency_spike": "Best-effort latency injection on the visits service, with a traffic fallback if Chaos Monkey is unavailable.", "error_burst": "Best-effort exception injection on the visits service, with the existing fallback behavior if Chaos Monkey endpoints are unavailable.", "throughput_drop": "A deliberately slower traffic profile intended to reduce throughput even if direct chaos hooks are unavailable."}
